# Bot-Pair as Primary Analytics Unit

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make bot+pair the primary unit of analysis in BotsAnalyticsPage — one sidebar entry, one overview row, and one detail view per bot-pair combination instead of per bot.

**Architecture:** Single file change to `src/components/BotsAnalyticsPage.jsx`. Derive a flat `botPairEntries` array (one element per bot-pair with trades) from `tradesByBotPair`. Sidebar, overview table, equity chart, and detail view all iterate `botPairEntries`. The per-pair breakdown table in `BotDetailView` is removed (redundant — already at pair level). `BotDetailView` is renamed `BotPairDetailView` and receives `bot`, `pair`, and pair-scoped `trades`.

**Tech Stack:** React 19, Recharts (already installed), Vitest 3.x (`npx vitest run`), no backend.
...
- Test command: `npx vitest run`. All 51 tests must pass.
- `selectedId` format: `'overview'` | `'{botId}:{pair}'` | `'manage'`.
- Sidebar entry label format: `'{bot.name} / {pair}'`.
- Overview table columns (in order): Bot / Pair / Trades / Win Rate / Net % Return / Avg % / Trade / Profit Factor / Best Day % / Worst Day %.
- Equity chart: one `<Line>` per bot-pair entry; `dataKey='{botId}:{pair}'`; `name='{bot.name} / {pair}'`.
...
| File | Change |
|------|--------|
| `src/components/BotsAnalyticsPage.jsx` | Single task — full rewrite of analytics logic, sidebar, overview, and detail view |

---

### Task 1: Refactor BotsAnalyticsPage to use bot-pair as primary unit

**Files:**
- Modify: `src/components/BotsAnalyticsPage.jsx`

**Interfaces:**
- Consumes (unchanged from previous tasks):
  - `resolveBotForTrade(trade, bots) → { bot, pair } | null` from `src/utils/botUtils.js`
  - `computeStats(trades)` and `computeCumulativePctSeries(trades, accounts)` from `src/utils/analytics.js`
  - `bot.pairMagicNumbers: { [pair: string]: number[] }`
- Props (unchanged): `{ bots, trades, accounts, onSaveBot, onDeleteBot }`

Read `src/components/BotsAnalyticsPage.jsx` before editing.

- [ ] **Step 1: Update the main `useMemo` to produce `botPairEntries`**

Replace the existing `useMemo` block (which produced `tradesByBot`, `tradesByBotPair`, `botsWithTrades`, `botsWithoutTrades`) with:
...
- [ ] **Step 2: Update `selectedId` parsing in the main render**

Replace the `BotDetailView` render block (the IIFE that looks up `bots.find(b => b.id === selectedId)`):
...
Replace the entire sidebar `<aside>` content:
...
- [ ] **Step 4: Rewrite `OverviewView` for bot-pair rows and equity lines**

Replace the `OverviewView` function entirely:
...
Update the `OverviewView` call site in `BotsAnalyticsPage` (currently passes `botsWithTrades` and `tradesByBot`):
...
- [ ] **Step 5: Rename `BotDetailView` to `BotPairDetailView` and update it**

Replace the entire `BotDetailView` function with `BotPairDetailView`. Changes from the old version:
- Signature: `function BotPairDetailView({ bot, pair, trades, accounts })`
- Header: show the pair as a prominent badge next to the bot name; show only magic numbers for this pair (`bot.pairMagicNumbers?.[pair] ?? []`)
- Remove the per-pair breakdown card entirely (we are already at pair level)
- Everything else (stats, equity curve, breakdown card, trade table) unchanged
...
Note: the trade table removes the `Symbol` column (redundant — we're already on a specific pair) and keeps Date, Account, Dir, PnL ($), PnL (%), Result.
...
git commit -m "feat: bot-pair as primary analytics unit (flat sidebar, per-pair overview rows)"
```
